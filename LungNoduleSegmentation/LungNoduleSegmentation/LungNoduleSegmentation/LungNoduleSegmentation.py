...
# for later make these try catch
...
class LungNoduleSegmentationWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def onSegmentationButton(self):
        volume = self.ui.segmentationComboBox.currentNode()
        ...
        segmentation = model.run(tensor_roi.to(torch.float32).unsqueeze(0).unsqueeze(0))

        # project back to original volume
        full_volume = self.ui.volumeComboBox.currentNode()
        ...
        for i in range(segmentation.shape[0]):
            for j in range(segmentation.shape[1]):
                for k in range(segmentation.shape[2]):
                    full_segmentation[i + conversion[0], j + conversion[1], k + conversion[2]] = segmentation[i, j, k]
        ...
        spacing = full_volume.GetSpacing()
        origin = full_volume.GetOrigin()
                # Create a vtkMatrix4x4 object to store the direction matrix
        ...
        # Apply the spacing, origin, and direction to the new labelmap
        ...
        print("Segmentation created with the specified spacing and origin.")
